...
def pause(screen):
    running = True
    button_continue = Button("Continue", 540, 305, screen)
    return_menu = Button("Return to the menu", 540, 385, screen)
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if button_continue.is_clicked(event.pos):
                    running = False
                elif return_menu.is_clicked(event.pos):
                    raise MenuError
...
def rule():
    running = True
    fon = load_image("rule.png")
    screen.blit(fon, (0, 0))
    back = Button("Back", 0, 0, screen)
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
...
def start_screen():
    try:
        running = True
        play_button = Button("Play", 540, 225, screen)
        rule_button = Button("Rules", 540, 345, screen)
        exit_button = Button("Exit", 540, 475, screen)
        while running:
            screen.fill(pygame.color.Color("grey"))
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if play_button.is_clicked(event.pos):
                        main()
                    elif rule_button.is_clicked(event.pos):
                        rule()
